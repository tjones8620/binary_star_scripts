...
class XrayProj:
    ...
    def create_png_dir(self):
        if not os.path.exists(self.png_dir):
            ...
            # input("Press enter to continue and clear this directory")
            # # Clean directory
    ...
    def create_imgs(self, i, ax, band="hard" ):
        ...
        image = ax.imshow(data, cmap=self.kwargs.get("cmap", "gist_heat"), origin="lower", 
        ...
        if self.trajectory_file:
            ...
            # print(star1_x, star1_y, star2_x, star2_y, star1_z, star2_z)
        ...
        st = r"$\phi$ = " + f"{phase:.3f}" 
        ax.text(0.7, 0.9, st, color="black", fontsize=8*5/4, transform=ax.transAxes, 
    ...
    def three_time_slice(self, colormap='viridis', d_phase = 0.005):
        ...
        for index in plot_indices:
            # print(index)
            image = self.create_imgs(index, ax=axes[num])
            num += 1
    ...
    def five_time_slice(self, colormap='viridis', d_phase = 0.005):
...
#     png_dir = os.path.join(home, "code/project/scripts/images/xray-proj")
...
#     # plot.evolution = plot.evolution[200:250]
#     plot.five_time_slice(d_phase=0.003)
...
def main(): 
    ...
    plot = XrayProj(data_dir, fits_dir, png_dir, start_time, trajectory_file=trajectory_file, vmin=0, vmax=2e-3, 
