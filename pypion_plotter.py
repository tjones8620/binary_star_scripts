...
class Plot_Functions:
    def __init__(self, path, image_dir, fluid_quantity, tolerance, plane):
        ...
        ########## Display number of levels in terminal ################
        ########## Categorizing data files into levels #################
        if len(level_list) == 1: 
            print('Simulation Info: Single level')
            catalog = []
            files = sorted(glob.glob(self.filename + '_0000.*.silo'))
            catalog.append(files)
        else:
            print(f'Simulation Info: {len(level_list)} levels')
            catalog = []
            for i in range(len(level_list)):
                files = sorted(glob.glob(f"{self.filename}_level{level_list[i]}_0000.*.silo"))
                catalog.append(files)
        ...
        self.evolution = np.array(catalog).T.tolist()
        ...
        os.chdir(image_dir)
        self.ImageDir = os.path.join(image_dir, f'SimulationPlots/{self.filename}/{self.Quantity}/{self.Surface}')
        try:
            # Create target Directory
            os.makedirs(self.ImageDir)  # changed mkdir to makedirs - creates nested folders
            print("Image directory" , self.ImageDir,  "created.")
        except FileExistsError:
            print("Image directory", self.ImageDir, "already exists.")
            ...
            print('Cleaning up the',  self.ImageDir, 'directory ...')
            for f in os.listdir(self.ImageDir):
                os.remove(os.path.join(self.ImageDir, f))
    ...
    def ThreeDSurfacePlotter(self, colormap='viridis', movie=False):
        ...
        if (self.Surface == 'XY'): xlabel = 'x'; x = 2; ylabel = 'y'; y = 1 ; z = 0 
        if (self.Surface == 'XZ'): xlabel = 'x'; x = 0; ylabel = 'z'; y = 2 ; z = 1
        if (self.Surface == 'YZ'): xlabel = 'y'; x = 1; ylabel = 'z'; y = 0 ; z = 2
